...
DEFAULT_EXCHANGE = "decision_agent.bizdata.events"
DEFAULT_ROUTING_KEY = "decision_agent.session.business_data.v1"
DEFAULT_SOURCE_SERVICE = "decision_agent_session"
DEFAULT_BIZDATA_MESSAGE_TYPE = "decision_agent.session.business_data"
...
def generate_bizdata_message(
    *,
    ...
    if content_text is None:
        content_text = _derive_bizdata_summary(schema_type_text, data)
...
def validate_bizdata_message(message: object) -> dict[str, Any]:
    if not isinstance(message, dict):
        raise ContractValidationError(["message must be a JSON object"])

    errors: list[str] = []
    _reject_unexpected_fields(message, _ALLOWED_BIZDATA_MESSAGE_FIELDS, "message", errors)
    _expect_non_empty_string(message.get("event_id"), "message.event_id", errors)
    if message.get("event_type") != DEFAULT_BIZDATA_MESSAGE_TYPE:
        errors.append(f"message.event_type must be {DEFAULT_BIZDATA_MESSAGE_TYPE!r}")
    ...
    if not isinstance(payload, dict):
        ...
        _reject_unexpected_fields(payload, _ALLOWED_BIZDATA_PAYLOAD_FIELDS, "message.payload", errors)
...
def _derive_bizdata_summary(schema_type: str, data: dict[str, Any]) -> str:
...
def generate_bizdata_message_from_markdown(
    *,
    markdown: object,
    conversation_id: object,
    turn_id: object,
    ...
    occurred_at: object | None = None,
    source_service: object = DEFAULT_SOURCE_SERVICE,
    schema_type: object = "text_message",
    now_ms: int | None = None,
    ...
    if event_id_text is None:
        event_id_text = (
            f"decision-agent.bizdata.{conversation_id_text}.{sequence_value}.{current_ms}"
        )
...
async def publish_bizdata_message(
    message: object,
    *,
    ...
    validated = validate_bizdata_message(message)
