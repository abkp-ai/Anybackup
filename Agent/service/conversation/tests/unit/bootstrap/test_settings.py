...
def test_settings_load_default_service_baseline() -> None:
    ...
    assert settings.decision_agent_bizdata_exchange == "decision_agent.bizdata.events"
    assert settings.decision_agent_bizdata_queue == "conversation.decision_agent.bizdata"
    assert settings.decision_agent_bizdata_routing_key == "decision_agent.session.business_data.v1"
